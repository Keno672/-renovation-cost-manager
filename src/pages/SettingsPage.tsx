...
import { Download, FileJson, FileSpreadsheet, LogOut, RotateCcw, Upload, Users } from 'lucide-react'
import { exportProjectCsv, exportProjectJson } from '../lib/export'
import { isProject } from '../lib/projectValidation'
...
export function SettingsPage({ project, onRename, onImport, onReset, userEmail, onSignOut }: SettingsPageProps) {
  ...
  return (
    <div>
      <PageHeader eyebrow="Project & data" title="Settings" description="Manage the shared project, keep a portable backup, or export a cost schedule for printing and sharing." />
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="panel p-5 sm:p-7"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-paper"><Users size={20} /></span><div><h2 className="font-display text-xl">Project details</h2><p className="text-xs text-ink/45">Shared securely through Supabase</p></div></div><form onSubmit={saveName} className="mt-6"><label><span className="label">Project name</span><input className="field" value={name} onChange={(event) => setName(event.target.value)} maxLength={80} /></label><button type="submit" className="btn-primary mt-4">Save project name</button></form><dl className="mt-7 grid grid-cols-2 gap-4 border-t border-black/5 pt-5 text-sm"><div><dt className="text-xs text-ink/45">Schedule items</dt><dd className="mt-1 font-bold">{project.workItems.length}</dd></div><div><dt className="text-xs text-ink/45">Currency & locale</dt><dd className="mt-1 font-bold">Euro · Ireland</dd></div></dl></section>
        ...
        <section className="panel p-5 sm:p-7"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-paper"><Upload size={20} /></span><div><h2 className="font-display text-xl">Import backup</h2><p className="text-xs text-ink/45">Restore a previous JSON export</p></div></div><p className="mt-5 text-sm leading-6 text-ink/55">Importing replaces the shared project for every authorised user. Export a backup first if you need to preserve it.</p><input ref={fileInput} type="file" accept="application/json,.json" className="hidden" onChange={importFile} /><button type="button" onClick={() => fileInput.current?.click()} className="btn-secondary mt-5"><Upload size={17} />Choose JSON file</button></section>

        <section className="rounded-2xl border border-rose-200 bg-rose-50/40 p-5 sm:p-7"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-white text-rose-600"><RotateCcw size={20} /></span><div><h2 className="font-display text-xl">Reset schedule</h2><p className="text-xs text-rose-700/60">This changes the shared project</p></div></div><p className="mt-5 text-sm leading-6 text-ink/55">Restore the full original schedule for every user and reset all costs, notes, contractors and statuses.</p><button type="button" onClick={reset} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-rose-700 px-4 text-sm font-bold text-white hover:bg-rose-800"><RotateCcw size={17} />Reset all project data</button></section>

        <section className="panel p-5 sm:p-7 lg:col-span-2"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[.1em] text-ink/40">Signed in as</p><p className="mt-1 font-semibold">{userEmail}</p></div><button type="button" onClick={onSignOut} className="btn-secondary"><LogOut size={17} />Sign out</button></div></section>
      </div>
    </div>
  )
}
